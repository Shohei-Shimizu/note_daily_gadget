    ...
    try {
        ...
        const EXP = `(() => {
            ...
            for (const doc of docs) {
                // "こんにちは" が含まれるテキストノードを探し（ただしterminal等を除外）
                ...
                while (node = walker.nextNode()) {
                    if (node.nodeValue && node.nodeValue.includes('こんにちは')) {
                         ...
                         while(checkNode) {
                             if(checkNode.classList && (checkNode.classList.contains('xterm') || checkNode.classList.contains('monaco-editor'))) {
                                 isValid = false;
                                 break;
                             }
                         ...
                         if (isValid) {
                             ...
                             return "Found valid text node 'こんにちは'. Ancestors:\\n" + classes.join('\\n');
                         }
                    }
                }
            }
            return 'not found (only terminal)';
        })()`;
