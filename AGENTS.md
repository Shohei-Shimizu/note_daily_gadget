...
>
> 全アカウント共通の商品DB（`shared/db/product.sqlite`）を構築中で、将来リサーチは「記事ごとの候補プール作成」から「カテゴリ単位で蔵書を積み、そこからテーマを選ぶ」方式へ変わる。設計は `shared/docs/product-db-design.md`、概要は親ワークスペース `CLAUDE.md` の「商品DB（蔵書）」章。
>
