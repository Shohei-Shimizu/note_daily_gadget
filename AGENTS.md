...
> [!IMPORTANT]
> **蔵書モデルへの移行が進行中。ただし現時点では以下の手順が正。**
>
> 全アカウント共通の商品DB（`shared/db/product.sqlite`、通称「蔵書」）を構築中で、リサーチは「記事ごとの候補プール作成」から「カテゴリ単位で蔵書を積み、そこからテーマを選ぶ」方式へ変わる。設計は `shared/docs/product-db-design.md`、フロー全体は `shared/docs/research-guide-core.md` の「蔵書モデルへの移行」章、概要は親ワークスペース `CLAUDE.md` の「商品DB（蔵書）」章。
>
> 移行後のフローと役割分担:
>
> ```
> ① 蔵書整理                      Codex    週次で定期実行。記事の予定とは無関係に回る
> ② テーマ決定                    Claude   蔵書に問い合わせ。テーマと候補商品群
> ③ ファクトチェック + サムネイル  Codex    再実測 → 機械選抜で件数確定 → サムネイル生成
> ④ タイトル確定 + 執筆            Claude   確定件数を入れてタイトル確定
> ⑤ 入稿                          Claude   note.com 下書き
> ```
>
> **Codex は「測る・集める・検証する」、Claude は「選ぶ・決める・書く」。** 人間はタイトルを決めない。決めるのは公開日と本数だけ（従来からそう）。
>
> **② テーマ決定は Codex から Claude へ移る。** 従来はCodexがリサーチしながらテーマを決めて予定表に記録していたが、そのとき見ていたのは**その記事のために作った使い捨ての候補プールだけ**だった。狭い母集団でテーマを立てて同じ母集団から商品を選ぶので、実在1件のテーマが通る（9/22 ロールスクリーン、8月の 9/19 スマートミラー）。新フローでは蔵書全体へ問い合わせてから決める。**変わったのは母集団であって、テーマの立て方ではない。**
>
> **件数は③の機械選抜が決める。②でも④でも先に宣言しない。** 先に件数を決めると、枠を埋めるために基準を下げることになり、Tier1ゼロの記事や同一商品の色違い重複が生まれる。
>
